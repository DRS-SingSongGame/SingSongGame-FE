...
const FlatLyricsGame = ({
  ...
  const addScoreAnimation = (
    ...
    setTimeout(() => {
      setGameState((prev) => ({
        ...prev,
        scoreAnimations: prev.scoreAnimations.filter(
          (anim) => anim.id !== newAnimation.id
        ),
      }));
    }, 2000);
  };
  ...
  const handleRoundStart = (payload: any) => {
    ...
    newAudio.oncanplaythrough = () => {
      ...
      if (delay > 1000) {
        setTimeout(() => setIsAudioLoading(false), delay - 1000);
      } else {
  ...
  const handleGameEnd = (data: any) => {
    ...
    aiResultSound
      .play()
      .catch((err) => console.error("airesult.wav 재생 실패:", err));
  ...
  return (
    <>
      {isAudioLoading && (
        <div className="fixed top-[30%] left-1/2 transform -translate-x-1/2 z-50 bg-black text-white px-6 py-3 rounded-2xl text-xl font-bold shadow-2xl animate-pulse">
          🎤 AI 음성 준비 중...
        </div>
      )}
