...
export const connectGameSocket = (roomId: string, callbacks: GameSocketCallbacks) => {
  ...
  stompClient.connect({}, (frame) => {
    ...
    stompClient?.subscribe(`/topic/room/${roomId}/round-start`, (response) => {
      callbacks.onRoundStart(JSON.parse(response.body));
    });

    stompClient?.subscribe(`/topic/room/${roomId}/answer-correct`, (response) => {
...
export const sendGameMessage = (roomId: string, senderId: string, senderName: string, message: string) => {
